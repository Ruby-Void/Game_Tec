...
PhysicsObject::PhysicsObject(Transform* parentTransform, const CollisionVolume* parentVolume, bool phasing) {
	transform		= parentTransform;
	volume			= parentVolume;
	phasingObject	= phasing;
	inverseMass		= 1.0f;
	elasticity		= 0.8f;
	friction		= 0.8f;
}
...
void PhysicsObject::UpdateInertiaTensor() {
	...
	Matrix3 invOrientation = Matrix3(q.Conjugate()), orientation = Matrix3(q);
